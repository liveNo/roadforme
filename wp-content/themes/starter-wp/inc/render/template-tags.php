...
function igthemes_posts_navigation() {
    ...
    if (get_theme_mod('numeric_pagination')) {
        ...
        echo paginate_links( array(
            ...
            'type'    => 'list',
            'prev_next'    => True,
            'prev_text'    => esc_html__('&#8592;', 'starter-wp'),
            'next_text'    => esc_html__('&#8594;', 'starter-wp'),
    ...
    <?php }// end pagination
        ...
        the_posts_navigation(array(
            'prev_text'          => __( 'Previous','starter-wp' ),
            'next_text'          => __( 'Next','starter-wp' ),
        ));
    }
    echo "</div>";
}
...
function igthemes_posted_on() {
    //AUTHOR
    $byline = sprintf(
          esc_html_x( apply_filters('igthemes-author','igthemes-author-text') . '%s', 'post author', 'starter-wp' ),
	...
	$posted_on = sprintf(
		esc_html_x( apply_filters('igthemes_date', 'igthemes_date_text') . '%s', 'post date', 'starter-wp' ),
...
function igthemes_entry_footer() {
	...
	if ( 'post' === get_post_type() ) {
		...
		if ( $categories_list && igthemes_categorized_blog() ) {
			printf( '<span class="cat-links">' . esc_html__( apply_filters('igthemes_cat_links', 'igthemes-cat-text') . '%1$s', 'starter-wp' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html__( ', ', 'starter-wp' ) );
		if ( $tags_list ) {
			printf( '<span class="tags-links">' . esc_html__( apply_filters('igthemes_tags_links', 'igthemes-tags-text') . '%1$s', 'starter-wp' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
	}
    //commnet
